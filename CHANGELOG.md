...
- Added PNG Copy/Cut for pixel selections, including OpenShop-local paste when
  the browser declines system clipboard permission and an explicit warning path.
...
### Fixed
- Added machine-checked runtime provenance for bundled dependencies; the pinned jsPDF report records that DOMPurify is not embedded or reachable through OpenShop's export paths.
